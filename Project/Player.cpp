...
AppStatus Player::Initialise()
{
	...
	if (data.LoadTexture(ResourceType::IMG_PLAYER, "images/eric.png") != AppStatus::OK)
	{
	...
	for (i = 0; i < 12; ++i)
	{
		if (i < 3) sprite->AddKeyFrame((int)PlayerAnim::BITE_RIGHT, { (float)i * n, 0, n, n });
		else if (i < 6) sprite->AddKeyFrame((int)PlayerAnim::BITE_LEFT, { (float)i * n, 0, n, n });
		else if (i < 9) sprite->AddKeyFrame((int)PlayerAnim::BITE_UP, { (float)i * n, 0, n, n });
		else sprite->AddKeyFrame((int)PlayerAnim::BITE_DOWN, { (float)i * n, 0, n, n });
	}
...
void Player::MoveY()
{
	...
	{
		pos.y += -PLAYER_SPEED;
		if (state == State::IDLE) StartWalkingLeft();//A CAMBIAR

		box = GetHitbox();
		if (map->TestCollisionWallLeft(box)) //A CAMBIAR
		{
			pos.y = prev_y;
			if (state == State::WALKING) Stop();
		}
	...
	{
		pos.y += PLAYER_SPEED;
		if (state == State::IDLE) StartWalkingRight();//A CAMBIAR

		box = GetHitbox();
		if (map->TestCollisionWallRight(box)) //A CAMBIAR
		{
			pos.y = prev_y;
			if (state == State::WALKING) Stop();
		}
